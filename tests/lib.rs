...
mod tests {
    ...
    /// The ephemeral `Close` gate: an *owned* crank (non-zero `authority`) may
    /// only be closed by that authority, because Magic refunds the vault rent to
    /// whoever signs the teardown. Unowned cranks stay permissionless.
    ///
    /// The gate runs before `process_close` and well before the Magic CPI, so
    /// all three cases below resolve without Magic being present. Getting past
    /// the gate shows up as `NotClosable` (5) — these cranks are healthy — while
    /// being stopped by it is `UnauthorizedAuthority` (4). That difference is
    /// the whole assertion.
    #[test]
    fn ephemeral_close_gates_owned_cranks_to_their_authority() {
        ...
        let (crank_pda, _bump) = find_crank(&SEED);
        let (system_program, system_program_acct) = keyed_account_for_system_program();
        let vault = Pubkey::new_from_array(hydra_api::consts::magic::EPHEMERAL_VAULT_ID.to_bytes());
        let magic = Pubkey::new_from_array(hydra_api::consts::magic::MAGIC_PROGRAM_ID.to_bytes());
